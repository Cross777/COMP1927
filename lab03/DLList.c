...
{
	if (L != NULL && L->curr != NULL) {
	   if (L->curr->prev != NULL) L->curr->prev->next = L->curr->next;
	   if (L->curr->next != NULL) L->curr->next->prev = L->curr->prev;
	   ...
	   if (L->curr->prev == NULL) L->first = L->curr;
	   if (L->curr->next == NULL) L->last = L->curr;
	   free(tmp);
	   L->nitems--;
	}
}
