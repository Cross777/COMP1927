...
{
   // function should fail if the original list (before insertion) is not sorted
   assert(L != NULL);
   ...
   if (L->first == NULL) {
      ...
      // middle
      struct IntListNode *testNode, *prevNode;
      for (testNode = L->first; testNode != NULL && v >= testNode->data; testNode = testNode->next) {
         prevNode = testNode; // to keep track of the previous node before testNode advances
      }
