...
int main()
{
    ...
    dv = newDracView("GGE.... SGE.... HGE.... MGE....", messages9);
    LocationID *path = NULL;
    int pathLength;
    printf("Madrid -> Paris: All\n"); 
    path = getDracShortestPath(dv, MADRID, PARIS, &pathLength,1,1); 
    free(path);
    printf("Edinburgh -> Athens: All\n"); 
    path = getDracShortestPath(dv, EDINBURGH, ATHENS, &pathLength,1,1);
    free(path);
    printf("Amsterdam -> Athens: Road only\n"); 
    path = getDracShortestPath(dv, AMSTERDAM, ATHENS, &pathLength,1,0);
    free(path);
    printf("Madrid -> Varna: Road only\n"); 
    path = getDracShortestPath(dv, MADRID, VARNA, &pathLength,1,0);  
    free(path);
    printf("Castle Dracula -> Paris: All\n"); 
    path = getDracShortestPath(dv, CASTLE_DRACULA, PARIS, &pathLength,1,1);  
    free(path);  
    disposeDracView(dv);
    return 0;
}
