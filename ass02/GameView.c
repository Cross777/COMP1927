...
static void processBuffer(GameView currentView, Link pQ, int *pQLen, 
                          ...
                          LocationID curr);
static void appendLocationsToQueue(GameView currentView, Link pQ, int *pQLen,
                                  LocationID *buffer, int size, 
                                  LocationID cur);
static void shift(Link pQ, int *pQLen, LocationID *location);
static void updatePrev (GameView currentView, Link pQ, int *pQLen, 
                        Link prev, LocationID cur);
// qsort function
                           ...
                           LocationID cur)
{
    ...
    updatePrev(currentView, pQ, pQLen, prev, cur);   
    #ifdef DEBUG_FINE
                                   ...
                                   int bufferLen, LocationID cur)
{
    ...
    for (i = 0; i < bufferLen; i++) { 
        ...
        if (cur == buffer[i]) continue;
        
        conns = connections(currentView->gameMap, cur, buffer[i], type);
        if (conns > MAX_TRANSPORT) {
            fprintf(stderr, "appendLocationsToQueue: connections \
                             return value invalid ( %d > 3 )", conns);
            abort();
        }
        //printList("type", type, MAX_TRANSPORT);
                        ...
                        Link prev, LocationID cur) 
{
    ...
    for (i = 0; i < *pQLen; i++) {
        pQ[i].dist = prev[cur].dist + 1; // adding in rail calc
        if (pQ[i].dist < prev[pQ[i].loc].dist) {
            prev[pQ[i].loc].dist = pQ[i].dist; 
            prev[pQ[i].loc].loc = cur;
...
void flipPath (LocationID *path, int l)
{
    int i = 0;
    LocationID tmp;
    while (i < l) {
