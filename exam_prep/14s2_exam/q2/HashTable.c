...
{
   /*// double the number of slots in the table
   int newN = ht->nslots * 2;
   ht = malloc(sizeof(ht)*2);
   ht->nslots = newN;
   ht->chains = malloc(newN*sizeof(List));
   int i;
   for (i = 0; i < newN; i++)
      ht->chains[i] = newList();

   // rehash all of the existing keys to their new slots (chains)
   int n;
   for (i = 0; i < newN; i++) {
      int *allvals = valuesFromList(ht->chains[i], &n);
      int j;
      printf("XXXXXX\n");
      for (j = 0; j < n; j++) {
         printf("%d ", allvals[j]);
